...
dbutils.fs.unmount("/mnt/wesley/creditcarddata")

// COMMAND ----------

display(dbutils.fs.ls("/mnt/wesley/creditcarddata"))
...
val AccessKey = ""
val AwsSecret = ""
val EncodedSecretKey =  URLEncoder.encode(AwsSecret, "UTF-8")

val AwsBucketName = "databricks-wes/dataset/Fintech/CreditCard"
val MountName = "mnt/wesley/creditcarddata"
dbutils.fs.mount(s"s3a://$AccessKey:$EncodedSecretKey@$AwsBucketName/", s"/$MountName")

// COMMAND ----------

// DBTITLE 1,CSV File contains 77,617,325 rows
val filePath = "dbfs:/mnt/wesley/creditcarddata/credit_card_data.txt"
println("Number of Rows: " + sc.textFile(filePath).count())

// COMMAND ----------

// DBTITLE 1,CSV File is 20.9 GB
val fileSize = dbutils.fs.ls("dbfs:/mnt/wesley/creditcarddata/credit_card_data.txt").map(_.size).sum / (Math.pow(1024.0,3))
println(f"Size of file in GB: $fileSize%2.2f")

// COMMAND ----------

// DBTITLE 1,Register Table (auth_data) From CSV
val rawAuth = sqlContext.read.format("csv").option("delimiter", "|").option("inferSchema", "true").load("dbfs:/mnt/wesley/creditcarddata/credit_card_data.txt")
